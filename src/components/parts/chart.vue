    ...
    created() {
        ...
        for (let i = 0; i < pointersArray.length; i++) {
            ...
            obj = {   
                ...
                y2: Math.round(this.linesCoordinates[1].y2 - this.linesCoordinates[1].y2 * quantity / 100) // 6 - место для отображения даты
            };
            ...
            this.columns.push({  
                ...
                color: isOdd ? 'firebrick' : 'dodgerblue',
                polygonCoordinates: "" + obj.x1 + " " + obj.y1 + "," + " " +
                                         obj.x1 + " " + obj.y2 + "," + " " +
                                         obj.x2 + " " + obj.y2 + "," + " " +
                                         obj.x2 + " " + obj.y1, // сформированная строка с координатами полигона
                quantityCoordinatesX: "" + (pointersArray[i] - (("" + item.quantity).length*0.5) * transformFS / 2), 
                quantityCoordinatesY: "" + (obj.y2 - transform), 
                quantityText: "" + item.quantity, // строка с количеством звонков
                dateCoordinatesX: "" + (pointersArray[i] - this.widthColumn), 
                dateCoordinatesY: "" + (obj.y1),
                //dateTransformRotate: "translate(-" + transform +", " + transform +") rotate(40 " + (pointersArray[i]) + " " + Math.round(obj.y1) + ")",
                dateTransformRotate: "translate(-" + transform + ", " + (transform * 3) +") rotate(90 " + (pointersArray[i]) + " " + Math.round(obj.y1) + ")",
                dateText: "" + dayStr + "." + monthStr + "." + yearStr // строка с датой звонков
            });
        }
    }
}
